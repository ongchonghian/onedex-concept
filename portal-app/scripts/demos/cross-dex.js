/* ============================================================
   ...
   · [data-cp-id="acme"]              for the SGBuildex counterparty row
   · [data-element-id="bill-of-lading"]  for the BoL leaf in the picker
   nth-of-type and onclick-attribute-substring selectors are not used.
...
(function (window) {
  ...
  const crossDex = {
    ...
    seed: (workspace) => {
      // Fresh start — clear agreement surfaces so the wizard's duplicate-element
      // guard doesn't fire when Marcus re-picks Bill of Lading. Pin Marcus on
      // SGTradex.
    ...
    steps: [
      ...
      { action: 'expect', target: '#dropdown-pop [data-demo="new-agreement-dropdown.direct-send"]' },

      { action: 'click', target: '#dropdown-pop [data-demo="new-agreement-dropdown.direct-send"]', dwell: 700 },

      // ---- Data picker — pick Bill of Lading (single, skips pack-fork) ----
      { action: 'expect', target: '.screen[data-screen="data-picker"].active [data-element-id="bill-of-lading"]' },

      { action: 'annotate',
        anchor: '.screen[data-screen="data-picker"].active [data-element-id="bill-of-lading"]',
      ...
      { action: 'click', target: '.screen[data-screen="data-picker"].active [data-element-id="bill-of-lading"]', dwell: 600 },
      ...
      // ---- Counterparty picker — highlight Acme (SGBuildex) ----
      { action: 'expect', target: '.screen[data-screen="cp-picker"].active [data-cp-id="acme"]' },

      { action: 'annotate',
        anchor: '.screen[data-screen="cp-picker"].active [data-cp-id="acme"] .dex-chip',
      ...
      { action: 'annotate',
        anchor: '.screen[data-screen="cp-picker"].active [data-cp-id="acme"]',
      ...
      { action: 'click', target: '.screen[data-screen="cp-picker"].active [data-cp-id="acme"]', dwell: 800, after: 1200 },

      // ---- Cross-DEX warning ----
      { action: 'expect', target: '.screen[data-screen="warn-inline"].active' },

      { action: 'annotate',
        anchor: '.screen[data-screen="warn-inline"].active [data-demo="warn-inline.heading"]',
