...
| Demo runtime controller | `runDemoFlow(flowId)` in `portal-app/scripts/demos/runtime.js` | Iterates a flow's declarative step array; owns the cursor, callout, control-bar lifecycle. Validates `rationale` field on every `annotate` step at registration time. |
