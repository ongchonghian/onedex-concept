...
**Element version**:
A published, immutable schema record for a Data Element identified by `(element_id, version)`. Versioning is external — bumping a version creates a new immutable record, never mutates the previous one. Existing Agreements keep whatever version they snapshotted at creation time per the **Snapshot** rule; new Agreements pick the latest published version. The catalogue page surfaces two CTAs that reflect this model: `+ New element` (greenfield identity) and `+ New version of…` (bump on an existing identity). Registration produces an Element version directly; there is no draft state on the element itself — Sarah's in-flight authoring is local autosave only, **deliberately distinct from Agreement-level Draft** (which lives in `agreement_draft`). The terminal commit on the Review tab is *"Publish v1.0"* / *"Publish v1.1"* — "Register" is retired as legacy CSM-era vocabulary. See [ADR 0026](./docs/adr/0026-agreement-snapshot-immutability-schema-upgrade.md), [ADR 0039](./docs/adr/0039-data-element-registration-admin-authored-single-page-flow.md).
_Avoid_: schema revision, draft version (a version is only ever published — no draft state exists on the element), element edit (the verb is *publish a new version*; no in-place edit per ADR 0026), DE registration (the legacy verb; use *publish a new element* or *publish a new version*)
