...
**Smart Start seed**:
The structured starting point produced by an **on-ramp** at the entry of the registration flow — `{ meta?, fields[] }` (see [`register-onramps.js:9`](./portal-app/scripts/register-onramps.js:9)). Four on-ramp sources today: Fork (clone an existing Element), Sample (paste/upload CSV/JSON/xlsx), Plain English (NL prompt), and Form/PDF (OCR/VLM extraction). The seed lands Sarah on the Schema tab with fields pre-populated; she can edit, delete, or add before publishing. The seed itself is *what* the on-ramp produced; **Smart Start assist** is what the suggestion engine does *with* the seed plus other grounding sources.
