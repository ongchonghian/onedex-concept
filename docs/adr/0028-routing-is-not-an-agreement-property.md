...
> **Amended by [ADR 0033](./0033-reactive-pitstop-scope-capture-with-inference.md):** the wizard scope-capture step is now skippable, the Composer Pitstop chip gains a capture mode for deferred scope, and both surfaces show a soft pre-fill derived from a within-group + history inference rule. The "captured inline during Agreement creation" stance below is widened to "captured at the moment of friction — either in the wizard or on first compose." The platform's "no assumption about why Pitstops exist" stance is preserved.
