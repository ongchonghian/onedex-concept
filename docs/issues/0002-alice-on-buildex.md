...
- [ ] Alice Ho user record added with `primaryOrgId: 'cosco'`
- [ ] `alice-cosco` affiliation row added with `dexRoles: { bx: 'Operation User' }`
...
- [ ] The existing TX team-inbox seed line *"Alice approved CrimsonLogic appointment for ABC Logistics"* is now backed by the real Alice user record (no display change; underlying reference updated)
