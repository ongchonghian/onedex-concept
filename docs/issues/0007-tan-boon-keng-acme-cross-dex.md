...
Seed Tan Boon Keng as Acme Construction's BX `Admin User` (Acme's primary DEX is BuildEx). Convert the existing Acme participant card's hardcoded substring *"primary DEX is `<strong>BuildEx</strong>`"* into a structured fact driven by `ORGS.acme.primaryDexId: 'bx'`. The [ADR 0012](../adr/0012-cross-dex-action-warning.md) cross-DEX warning modal now names Tan Boon Keng as the Acme contact when Marcus initiates a cross-DEX action against Acme.
...
- [ ] Acme participant card's *"primary DEX is BuildEx"* line reads from `primaryDexId`, not a hardcoded substring
